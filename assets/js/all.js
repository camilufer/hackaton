...
function printMovie(){
	...
	for (var i in peliculas){
		...
		.then((response) => {
			...
			let movieId= response.data.imdbID
...
/* for (var j in peliculas){
			if (idbtn === peliculas[j].idTeoria){
				var titlePelicula=peliculas[j].title
				axios.get('http://www.omdbapi.com?t='+ titlePelicula + '&apikey=a7a10611')
				.then((response) => {
						let movieTitle= response.data.Title; 
						let movieActors= response.data.Actors;
						let movieAward= response.data.Awards; 
 ...
 function imprimirReview(){
	...
	for ( var i in peliculas){
		...
		.then((response) => {
			...
			$("#box-review").append('<div class="row esconder">'+
													      '<div class="col s12">'+
